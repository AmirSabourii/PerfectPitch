...
export default function PlatformAdminLoginPage() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.isPlatformAdmin) {
                    // User is platform admin, redirect to intended page
                    const returnUrl = searchParams.get('returnUrl') || '/platform-admin'
                    router.push(returnUrl)
                } else {
                    setError('You do not have platform admin access. Your email is not authorized.')
                }
            } else {
                setError('Unable to verify platform admin access. Please try again.')
            }
        } catch (err) {
            console.error('Error checking platform admin access:', err)
    ...
    const handleLogin = async () => {
        try {
            ...
            // checkPlatformAdminAccess will be called by useEffect when user is set
        } catch (err: any) {
        ...
        return (
            <div className="min-h-screen bg-[#030303] flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="mt-4 text-zinc-400 text-sm">Verifying platform admin access...</p>
                </div>
            </div>
        )
    }
        ...
        <div className="min-h-screen bg-[#030303] text-white font-sans selection:bg-white/20 flex flex-col items-center justify-center relative overflow-hidden">
            ...
            >
                <div className="text-center mb-12">
                    ...
                    <h1 className="text-3xl font-medium tracking-tight mb-2">Platform Admin</h1>
                    <p className="text-zinc-500">Sign in with your platform admin account.</p>
                </div>
                ...
                <div className="bg-[#0A0A0A] border border-white/10 rounded-3xl p-8 shadow-2xl">
                    ...
                    <p className="mt-6 text-center text-xs text-zinc-600">
                        Only authorized platform admin emails can access this area.
                    </p>
                </div>

                <div className="mt-6 text-center">
                    <Link href="/admin/login" className="text-sm text-zinc-500 hover:text-zinc-400 transition-colors">
